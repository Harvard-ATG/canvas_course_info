...
class ICommonsApi(object):
    def __init__(
        ...
        self.base_url = urlparse.urljoin(
            icommons_base_url.rstrip("/") + "/", api_path.lstrip("/")
        )
    ...
    def _get_resource(
        self, type_, id_=None, sub_resource_=None, collection=False, **kwargs
    ):
        """
    ...
    def _get_school(self, school_id, **kwargs):
        ...
        try:
            ...
            logger.exception(
                "Unable to validate school %s returned from the icommons api.", rv
            )
            raise ICommonsApiValidationError(str(e))
        return rv
    ...
    def _course_info_instructor_list(self, course_instance_id, **kwargs):
        instructors = None
        people_list = self._get_resource(
            "course_instances", course_instance_id, "people", collection=True, **kwargs
        )
        try:
            instructors = []
    ...
    def get_course_info(self, course_instance_id):
        ...
        if course_info is None:
            ...
            try:
                ...
                logger.debug(
                    log_msg.format(course_instance_id, json.dumps(course_info))
                )
                cache.set(cache_key, course_info)
            except Exception:
                logger.exception(
                    "Error retrieving course instance by id %s", course_instance_id
                )
        return course_info
        ...
        if course_info is None:
            ...
            try:
                ...
                if len(course_instance):
                    course_instance_id = self._get_course_instance_id(course_instance)
    ...
    def get_course_info_instructor_list(self, course_instance_id=None):
        ...
        """
        cache_key = CACHE_KEY_COURSE_PEOPLE_BY_COURSE_INSTANCE_ID.format(
            course_instance_id
        )
        course_staff_info = cache.get(cache_key)
        if course_staff_info is None:
            try:
                course_staff_info = self._course_info_instructor_list(
                    course_instance_id=course_instance_id
                )
                log_msg = "Caching course info people for course_instance_id {}: {}"
                logger.debug(
                    log_msg.format(course_instance_id, json.dumps(course_staff_info))
                )
    ...
    def _get_course_instance_id(self, course_instances):
        ...
        if len(primary_cids) == 1:
            ...
            error_msg = (
                "Multiple possible primary courses found, cannot determine primary"
            )
