...
mock_course_info = {
    "course": {"registrar_code_display": "ABC 1234", "school_id": "abc"},
...
mock_staff_data_from_api = [
    {
        ...
        "source": "xmlfeed",
        "role": {
            "role_name": "Course Head",
            "canvas_role": "Course Head",
            "role_id": 1,
        },
        "seniority_sort": 1,
        "user_id": "12121212",
...
mock_school_info = {"title_long": "Harvard School of ABCs"}
...
class DisplayTests(TestCase):
    ...
    api_course_mock_without_instructor = MagicMock(
        return_value=mock_course_info_without_instructor_display
    )
    ...
    def test_course_code(self, api_mock):
        ...
        client = Client(HTTP_REFERER=self.referer_string)
        response = client.get(
            "/course_info/widget.html?f=course.registrar_code_display"
        )
    ...
    def test_editor_partial_field_data(self, api_mock):
        ...
        self.assertContains(
            response,
            "course_info_checkbox",
            count=len(list(mock_course_info.keys())) - 1,
        )
    ...
    def test_widget_alternate_instructor_display(self, api_mock):
        ...
        api_mock.get_course_info_by_canvas_course_id = (
            self.api_course_mock_without_instructor
        )
        ...
        self.assertContains(
            response, mock_staff_data_from_api[0]["profile"]["name_first"]
        )
    ...
    def test_sort_and_format_instructor_display_names(self, api_mock):
        ...
        expected_instructor_name_format = (
            "user1_first user1_last, user3_first"
    ...
    def test_instructor_display_format_for_two_instructors(self, api_mock):
        ...
        expected_instructor_name_format = (
            "user1_first user1_last and user2_first user2_last"
        )
        result = sort_and_format_instructor_display(staff_data)
        self.assertEqual(str(result), expected_instructor_name_format)
...
class IntegrationTests(TestCase):
    ...
    def setUpClass(cls):
        ...
        cls.referer_header = "{}/courses/{}".format(
            cls.base_url, integration_test_course_info["canvas_course_id"]
        )
    ...
    def test_instructor_list_multiple_pages(self, session_get_mock):
        ...
        def results_generator(page_count=1):
            for index in range(page_count):
                yield {
                    "next": "fakeurl" if index < page_count - 1 else None,
